...
audio_model = tf.keras.models.load_model('my_audio_model_v2.h5', compile=True)
video_model.compile(tf.keras.optimizers.Adam(), loss='mse')
...
while (True):
    ...
    # add text to frame
    cv2.putText(img, str('Combined: ' + class_names[np.argmax(temp_predictions[0])]), (0, img.shape[0]), cv2.FONT_HERSHEY_PLAIN, 2,
                (0, 255, 0))
